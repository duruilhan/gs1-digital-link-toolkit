...
Bu liste, projeyi devralacak kişinin önce ele alması gereken sıraya göre düzenlenmiştir. İlk dört madde gerçek bir kullanıcı isteğinde görünür sonuç üretebilir; son iki madde ise mevcut davranıştan çok kanıt ve kapsam eksikliğidir.

1. **Sıkıştırılmış URI üretimi yok (kullanıcı etkisi yüksek).** Kütüphane sıkıştırılmış Digital Link URI'larını çözebilir ama üretemez. Sıkıştırılmış bir barkod/URL üretmek isteyen tüketici bu işlevi başka bir araçla yapmak zorundadır. Çözme desteğini tamamlayacak üretim yönü önceliklidir.
2. **Sıkıştırma başlık kapsamı eksik (kullanıcı etkisi yüksek).** Çözümleyicideki başlık tablosu standardın tüm başlık ve AI birleşimlerini değil, mevcut katalog ve test verisindeki kümeyi kapsar. Bu küme dışındaki geçerli bir sıkıştırılmış adres reddedilir; yeni başlıklar ve örneklerle kapsamın genişletilmesi gerekir.
3. **Dil eşleşmesi tek yönlü (kullanıcı etkisi orta).** Hedef dili `tr-TR`, istek başlığı `Accept-Language: tr` olduğunda eşleşme oluşmaz ve seçim varsayılan hedefe düşer. Ters yönde, daha ayrıntılı istek dili daha genel hedef dili kapsayabilir. Türkçe gibi genel bir dil tercihi için bölgesel hedef bekleyen kullanıcı yanlış sayfaya yönlenebilir.
4. **Bağlantı tipi geri dönüşü beklenmedik sonuç verebilir (kullanıcı etkisi orta).** Açıkça istenen bağlantı tipi için dil veya medya tipi eşleşmezse seçim varsayılan hedefe düşer. Varsayılan hedef farklı bir bağlantı tipine sahip olabilir. Bu bilinçli bir geri dönüş davranışıdır; devralan kişi, açık bağlantı tipi isteğinde `406` dönmenin daha doğru olup olmadığını ürün gereksinimine göre kararlaştırmalıdır.
5. **CSET 82 karakter kanıtı eksik (test güvencesi).** Test verisi üreticisi çift tırnak, tek tırnak, açma parantezi, kapama parantezi ve yıldız üretmez. Bu nedenle 2.000 girdilik fark testi ve 500 girdilik sıkıştırma çözme testi bu beş karakter için kanıt oluşturmaz; üretici genişletilip ayrı regresyon örnekleri eklenmelidir.
6. **Eğik çizgili değer için yalnız tek örnek doğrulandı (test güvencesi).** AI 10 değeri `LOT/1` olan tanımda eğik çizgi `%2F` olarak kodlanır; aynı kodlanmış yolla istek geldiğinde kayıt bulundu ve `307` döndü. Bir sorun görülmedi, ancak yalnız tek örnek üzerinden kanıt vardır; daha fazla ayrılmış karakter kombinasyonuyla regresyon testleri eklenmelidir.
